...
class Cantilever(object):
    ...
    def analyse(self, loads):
        ...
        # for nll constraint formulation in optimisation (g(x) <= 0):
        # compute g(x) = stress/R - 1 = 1/RF - 1
        d = {
            'wmax':     max([abs(self.w_max(F)) for F in loads]),
            'rf_t_uf':  min([self.rf_t_uf(F) for F in loads]),
            'rf_t_lf':  min([self.rf_t_lf(F) for F in loads]),
            'rf_c_uf':  min([self.rf_c_uf(F) for F in loads]),
            'rf_c_lf':  min([self.rf_c_lf(F) for F in loads]),
            'rf_lb_uf': min([self.rf_lb_uf(F) for F in loads]),
            'rf_lb_lf': min([self.rf_lb_lf(F) for F in loads]),
            'rf_s_web': min([self.rf_s_web(F) for F in loads]),
            'rf_wb':    min([self.rf_wb(F) for F in loads]),
            'rf_lat':   min([self.rf_lat(F) for F in loads]),
            'Fmax':     max([abs(F) for F in loads]),
            'mass':     self.mass(),
            'cost':     self.cost(),
            'area':     self.area
        }
